...
func TestReElection(t *testing.T) {
	...
	fmt.Printf("Test: disconnectiong %v...\n", leader1)
	cfg.disconnect(leader1)
	cfg.checkOneLeader()

	// if the old leader rejoins, that shouldn't
	// disturb the old leader.
	fmt.Printf("Test: reconnectiong %v...\n", leader1)
	cfg.connect(leader1)
...
func TestFailNoAgree(t *testing.T) {
	...
	// repair failures
	fmt.Printf("Test: connecting %v, %v, %v\n", (leader+1)%servers, (leader+2)%servers, (leader+3)%servers)
	cfg.connect((leader + 1) % servers)
	...
	for try := 0; try < 5; try++ {
		...
		for ii := 0; ii < iters; ii++ {
			...
			go func(i int) {
				...
				fmt.Printf("Added new log index: %v\n", i)
				is <- i
			}(ii)
		}
		...
		for index := range is {
			...
			fmt.Printf("Checking index: %v, cmd %v\n", index, cmd)
			if ix, ok := cmd.(int); ok {
				if ix == -1 {
...
func TestBackup(t *testing.T) {
	...
	fmt.Printf("Test: 2 disconnecting %v, %v\n", (leader1+0)%servers, (leader1+1)%servers)
	cfg.disconnect((leader1 + 0) % servers)
	cfg.disconnect((leader1 + 1) % servers)

	// allow other partition to recover
	fmt.Printf("Test: 3 reconnecting %v, %v, %v\n", (leader1+2)%servers, (leader1+3)%servers, (leader1+4)%servers)
	...
	fmt.Printf("Test: 4 disconnecting %v\n", other)
	...
	fmt.Printf("Test: 6 reconnecting everyone\n")
	// now everyone
	for i := 0; i < servers; i++ {
		cfg.connect(i)
	}
...
func TestPersist3(t *testing.T) {
	...
	cfg.one(102, 2)
	fmt.Printf("Test: crashing leader and one follower %v, %v\n", (leader+0)%servers, (leader+1)%servers)
	cfg.crash1((leader + 0) % servers)
	cfg.crash1((leader + 1) % servers)
	fmt.Printf("Test: reconnecting %v\n", (leader+2)%servers)
	cfg.connect((leader + 2) % servers)
	fmt.Printf("Test: rebooting leader %v\n", (leader+0)%servers)
	cfg.start1((leader + 0) % servers)
	cfg.connect((leader + 0) % servers)

	cfg.one(103, 2)

	fmt.Printf("Test: rebooting %v\n", (leader+1)%servers)
